...
DF1=2*SW*W+2*ST*W*(P.*R)'+2*ST*W*(P.*R);
DF1=DF1/norm(DF1);
W=W-r1*DF1;
...
DF3=2*SW*W2+2*ST*W2*R2'*R2*T+2*ST*W2*T*R2'*R2-4*ST*W2*R2'*R2;
...
figure(1)
plot(Cost1(131:Iterations),'r-*')

 
figure(2)
plot(Cost2(131:Iterations),'r-*')

figure(3)
plot(Cost3(131:Iterations),'r-*')

figure(4)
plot(Cost4(131:Iterations),'r-*')
%ylabel('The value of cost function');
%xlabel('Iterations');
 
 
figure(5)
plot(Cost5(151:Iterations),'r-*')

figure(6)
plot(Cost6(201:Iterations),'r-*')
